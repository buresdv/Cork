    ...
    var body: some View {
        VStack(alignment: .leading) {
            Text("Install Packages")
                .font(.headline)
                .padding(.leading)
            /// Clear all selected items when the user looks for a different package
            TextField("Search For Packages...", text: $packageRequested, onEditingChanged: { _ in
                foundPackageSelection = Set<UUID>()
            })
            .padding(.horizontal)
            ...
            if isShowingListLoader {
                ...
                InstallProgressTrackerView(
                    progress: $installationProgressTracker.progressNumber,
                    currentlyInstallingPackage: $installationProgressTracker.packageBeingCurrentlyInstalled)
                ...
                if foundPackageSelection.isEmpty {
                    Button {
                        ...
                        Task {
                            ...
                            do {
                                let foundFormulae: [String] = try await searchForPackage(
                                    packageName: packageRequested, packageType: .formula
                                )
                                let foundCasks: [String] = try await searchForPackage(
                                    packageName: packageRequested, packageType: .cask
                                )

                                for formula in foundFormulae {
                                    searchResultTracker.foundFormulae.append(
                                        SearchResult(packageName: formula, isCask: false)
                                    )
                                }
                    ...
                    HStack {
                        Button {
                            ...
                            let selectedPackages: [String] = getPackageNamesFromUUID(
                                selectionBinding: foundPackageSelection, tracker: searchResultTracker
                            )

                            for selectedPackage in selectedPackages {
                                PackageDetailWindow(
                                    package: selectedPackage,
                                    tracker: searchResultTracker, brewData: brewData
                                ).openNewWindow(with: "Detail - \(selectedPackage)")
                            }
                        ...
                        Button {
                            ...
                            installSelectedPackages(
                                packageArray: getPackageNamesFromUUID(
                                    selectionBinding: foundPackageSelection, tracker: searchResultTracker
                                ),
                                tracker: installationProgressTracker, brewData: brewData
                            )
        ...
        .alert("Error installing package",
               isPresented: $installationProgressTracker.isShowingInstallationFailureAlert) {
