        ...
        if let package = appState.packageTryingToBeUninstalledWithSudo
        {
            VStack(alignment: .leading)
            {
                ComplexWithImage(image: Image(localURL: URL(filePath: "/System/Library/CoreServices/KeyboardSetupAssistant.app/Contents/Resources/AppIcon.icns"))!)
                {
                    ...
                    {
                        Text("add-package.uninstall.requires-sudo-password-\(package.name(withPrecision: .precise))")
                            .font(.headline)
                ...
                Text("add.package.uninstall.requires-sudo-password.terminal-instructions-\(package.name(withPrecision: .precise))")
                    .font(.subheadline)
...
private struct ManualUninstallInstructions: View
{
    ...
    {
        return "brew uninstall \(package.type == .cask ? "--cask" : "") \(isPurgingPackage ? "--zap" : "") \(package.name(withPrecision: .precise))"
    }
