...
struct SidebarContextMenu: View
{
    ...
    {
        if outdatedPackagesTracker.allDisplayableOutdatedPackages.contains(where: { $0.package.getPackageName(withPrecision: .precise) == package.getPackageName(withPrecision: .precise) })
        {
            return true
        }
        else
