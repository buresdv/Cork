...
class BrewPackageDetails: ObservableObject
{
    ...
    // MARK: - Init
    init(name: String, description: String, homepage: URL, tap: BrewTap, installedAsDependency: Bool, dependents: [String]? = nil, dependencies: [BrewPackageDependency]? = nil, outdated: Bool, caveats: String? = nil, pinned: Bool)
    {
        self.name = name
    ...
    func changePinnedStatus() async throws
    {
        if self.pinned
        {
            let pinResult = await shell(AppConstants.brewExecutablePath, ["pin", self.name])
        ...
        {
            let unpinResult = await shell(AppConstants.brewExecutablePath, ["unpin", self.name])
